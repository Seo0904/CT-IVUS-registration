...
class Visualizer():
    ...
    def _wandb_log_visuals(self, visuals, epoch, global_step=None, split='train'):
        ...
        if payload:
            # Do not pass an explicit wandb step: end-of-epoch の val 画像 / val 指標 / lr は
            # train 最後のログと同じ total_iters を共有するため、step= を渡すと wandb の
            # monotonic-step ルールで後者が黙って捨てられる。total_iters を payload に入れて
            # define_metric(step_metric='total_iters') に x 軸を任せる。
        ...
        if self.wandb.enabled:
            ...
            for k, v in losses.items():
                try:
                    ...
                    pass
            # step= を渡さない: val/lr など end-of-epoch のログは train 最後と同じ
            # total_iters になり、monotonic-step ルールで wandb に捨てられてしまう。
            # total_iters は payload に含めて step_metric で x 軸を駆動する。
            self.wandb.log(payload)
    ...
    def wandb_log_files_as_images(self, image_paths, global_step=None, split='train', prefix='debug', epoch=None):
        ...
        for p in image_paths:
            try:
                ...
                continue
        if payload:
            # step= は渡さない (上記と同じ monotonic-step ドロップ回避)。
            self.wandb.log(payload)
