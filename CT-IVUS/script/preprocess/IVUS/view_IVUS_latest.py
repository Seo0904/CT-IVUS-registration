...
def view_ht() -> None:
	...
	for patient_dir in sorted(src_dir.iterdir()):
		...
		# HT は IVUS or OCT files 直下のファイルのみを DICOM として処理する
		files_direct = [
		...
		if not files_direct:
			print(f"  [WARN] No DICOM-like files directly under: {ivus_root}")
			continue
...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="CCTI-calc-IVUS_OCT 内の IVUS DICOM を可視化して PNG または TIFF 保存するスクリプト",
	)
	parser.add_argument(
		"--target",
		"-t",
		choices=["HT", "KH", "HK", "KY", "KN", "MN", "SH", "AC", "NT", "NTH", "WJ"],
		required=True,
	...
	parser.add_argument(
		"--save-format",
		"-f",
		choices=["png", "tiff", "meta"],
		default="png",
		help=(
			"保存形式を指定 (png / tiff / meta)。"
			"png の場合は /workspace/data/preprocessed/IVUS、"
			"tiff の場合は /workspace/data/preprocessed/IVUS_tiff、"
			"meta の場合は /workspace/data/preprocessed/IVUS_meta に保存される。"
		),
	)
...
def main() -> None:
	global CURRENT_SAVE_DIR, SAVE_SEQUENTIAL_FUNC, SAVE_MODE

	args = parse_args()

	# 保存形式に応じて出力ルートと保存関数 / モードを切り替える
	if args.save_format == "png":
		CURRENT_SAVE_DIR = BASE_SAVE_DIR_PNG
		SAVE_SEQUENTIAL_FUNC = save_dicom_array_sequential
		SAVE_MODE = "image"
	elif args.save_format == "tiff":
		CURRENT_SAVE_DIR = BASE_SAVE_DIR_TIFF
		SAVE_SEQUENTIAL_FUNC = save_dicom_array_sequential_tiff
		SAVE_MODE = "image"
	elif args.save_format == "meta":
		CURRENT_SAVE_DIR = BASE_SAVE_DIR_META
		# 画像は保存しないので SAVE_SEQUENTIAL_FUNC は未使用だが、ダミーとして PNG 用を入れておく
		SAVE_SEQUENTIAL_FUNC = save_dicom_array_sequential
		SAVE_MODE = "meta"
	else:
	...
	print(f"\nAll outputs saved under: {CURRENT_SAVE_DIR}")
