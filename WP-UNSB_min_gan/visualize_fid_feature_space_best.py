...
def reduce_and_plot(features, labels, files, out_dir, name, epoch,
    ...
    for key, reducer in methods.items():
        ...
        plt.ylabel(f"{key}-2")
        plt.tight_layout()
        out_path_no = os.path.join(out_dir, f"fid_featspace_{key}_train_{epoch}_no_overlap.png")
        plt.savefig(out_path_no, dpi=150)
        plt.close()
        print(f"Saved: {out_path_no}")
