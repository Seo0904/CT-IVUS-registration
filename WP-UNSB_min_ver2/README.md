...
## Logging (W&B)
This repo supports optional Weights & Biases logging for scalars (loss/metrics/FID/LR) and visuals.

Example:

```
python train.py --dataroot ./datasets/horse2zebra --name h2z_SB --mode sb \
  --lambda_SB 1.0 --lambda_NCE 1.0 --gpu_ids 0 \
  --use_wandb --wandb_project WP-UNSB_ver2
```

Notes:
- If you are in a headless/container environment, consider `--wandb_mode offline` (or set `WANDB_MODE=offline`).
- Image logging frequency is controlled by `--wandb_image_freq`.

## Plotting metrics from loss_log.txt
Training writes `loss_log.txt` under `checkpoints/<name>/`. You can plot all logged keys (including `val_*`) via:

```
python plot_loss_log.py --exp_dir checkpoints/<name>
```
