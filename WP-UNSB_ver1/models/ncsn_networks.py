...
class ResnetBlock_cond(nn.Module):
    ...
    def forward(self, x,time_cond,z):
        ...
        for n,layer in enumerate(self.conv_block):
            out = layer(out)  # x ではなく out を引き継ぐ（バグ修正）
            if n==0:
...
def get_norm_layer(norm_type='instance'):
    ...
    if norm_type == 'batch':
        ...
        # eps=1e-5(デフォルト)だと全画素均一(ゼロフレームなど)で分散=0→NaNになるため大きめに設定
        norm_layer = functools.partial(nn.InstanceNorm2d, affine=False, track_running_stats=False, eps=1e-3)
    elif norm_type == 'none':
