...
def sequence_ot_loss_torch(
    ...
    if use_identity_plan:
        if T == N:
            # 真の意味での単位行列対応
            P = torch.eye(T, device=fake_seq.device, dtype=fake_seq.dtype) / T
        else:
            # T != N のときは対角っぽい固定対応
            P = torch.zeros((T, N), device=fake_seq.device, dtype=fake_seq.dtype)
            for i in range(T):
                j = round(i * (N - 1) / max(T - 1, 1))
                P[i, j] = 1.0 / T
    else:
