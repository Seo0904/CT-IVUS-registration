...
# 再開対象の実験ID（必要なら環境変数で上書き）
RESUME_DATE=${RESUME_DATE:-20260410_170441}

# 再開開始epoch（必要なら環境変数で上書き）
RESUME_EPOCH=${RESUME_EPOCH:-175}

# ログ用の現在時刻
NOW=$(date +"%Y%m%d_%H%M%S")

# 再開元ディレクトリ（既存runを使う）
RESULT_DIR="${WORKSPACE_DIR}/data/experiment_result/UNSB/moving-mnist/${RESUME_DATE}"
...
WANDB_PROJECT=${WANDB_PROJECT:-UNSB}
WANDB_ENTITY=${WANDB_ENTITY:-}
WANDB_MODE=${WANDB_MODE:-offline}
WANDB_GROUP=${WANDB_GROUP:-moving-mnist}
WANDB_TAGS=${WANDB_TAGS:-moving-mnist,UNSB}
WANDB_IMAGE_FREQ=${WANDB_IMAGE_FREQ:-2000}
WANDB_RUN_NAME=${WANDB_RUN_NAME:-${NAME}_resume${RESUME_EPOCH}_${NOW}}
...
echo "Checkpoints Dir: ${RESULT_DIR}"
echo "GPU: ${GPU_IDS}"
echo "======================================"

if [[ ! -d "${RESULT_DIR}/${NAME}" ]]; then
  echo "[ERROR] Resume checkpoint directory not found: ${RESULT_DIR}/${NAME}"
  echo "        Set RESUME_DATE or NAME correctly."
  exit 1
fi
...
Resume:
  Continue Train: true
  Load Epoch: latest
  Start Epoch Count: ${RESUME_EPOCH}
EOF
