...
if __name__ == '__main__':
    ...
    for i, (data,data2) in enumerate(zip(dataset,dataset2)):
        ...
        if hasattr(model, 'fake_B') and hasattr(model, 'real_B'):
            ...
            fake_B_in, fake_B_meta = flatten_seq(fake_B)
            real_B_in, real_B_meta = flatten_seq(real_B)
            
            sample_metrics = {
                'index': i,
                'path': img_path[0] if isinstance(img_path, list) else str(img_path),
                'SSIM': ssim(fake_B_in, real_B_in),
                'L2': l2_loss(fake_B_in, real_B_in),
                'L1': l1_loss(fake_B_in, real_B_in),
                'PSNR': psnr(fake_B_in, real_B_in)
            }
            all_metrics.append(sample_metrics)
            metrics.update(fake_B_in, real_B_in)
