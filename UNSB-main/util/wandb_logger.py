...
class WandbLogger:
    ...
    def __init__(self, opt: Any):
        ...
        project = getattr(opt, 'wandb_project', None) or 'UNSB-main'
    ...
    def _define_default_metrics(self):
        ...
        try:
            try:
                self._wandb.define_metric('total_iters', hidden=True)
            except TypeError:
                self._wandb.define_metric('total_iters')
            self._wandb.define_metric('train/*', step_metric='total_iters')
            self._wandb.define_metric('val/*', step_metric='total_iters')
        except Exception as e:
